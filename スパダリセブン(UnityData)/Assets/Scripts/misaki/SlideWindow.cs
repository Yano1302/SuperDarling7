...
public partial class SlideWindow : MonoBehaviour
{
    ...
    private void SlideChildren(int value, float move)
    {
    ...
    /// </summary>
    /// <param name="move">移動値</param>
    private void PhaseSlide(float move)
    {
...
public partial class SlideWindow
{
    /// --------変数一覧-------- ///
    ...
    private bool openMenu = false; // メニューを開いているかどうか

    private float maxParentLength = 0; // 親オブジェクトの全長
    private float parentOffsetMin_x = 0; // 親オブジェクトのleft初期値
    private float[] phase; // 次フェーズに進める目標値
    private float phasePoint = 0; // 次フェーズに進むかどうかの判定で使うフロート値

    private RectTransform rectTransform; // 自身のRectTransform変数

    private RectTransform[] rectTransforms; // RectTransform配列

    [SerializeField] private UIManager uiManager; // UIManager変数

    [SerializeField] private AudioManager audioManager; // AudioManager変数

    [SerializeField] private SceneManager sceneManager; // SceneManager変数

    private SLIDESTATE slideState; // SLIDESTATE変数
