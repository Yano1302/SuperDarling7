...
/// <summary>
/// Enum関連についてのエディタ拡張を行っているクラスです。インスペクター上の表示を変更し、管理しやすくします。以下使用例です
/// 
///〇 [EnumIndex(typeof(列挙型名))]配列型の変数 
///　
/// ・列挙型と配列の添え字を連動させることが出来ます。
/// 
///  例) //列挙型
///     enum CharactorType
///     {
...
///     }
///     //変数宣言
///     [EnumIndex(typeof(CharactorType))]public GameObject[] CharactorObjects;     
///     
///                              インスペクター上
///        // 通常 //                                  // EnumIndex //
///     CharactorObjects                            CharactorObjects
///     Element 0 [None(GameObject)]       　       Player    [None(GameObject)]
///     Element 1 [None(GameObject)]                Monster   [None(GameObject)]
///             ・                                  　↑     ・
///             ・                 　　ここが列挙子と対応する・
///             ・                                           ・
///　
/// 
/// 〇　[EnumAction(typeof(列挙型名))]
/// 　
/// ・ボタン等にアタッチする関数(int型の引数)　でボタンの引数に列挙型を指定できます(int型にキャストされたものが引数として受け取れます)
/// 
///  例)
///  Class A
///  {
///       //ボタンにアタッチする為の関数
///       例)[EnumAction(typeof(CharactorType))]public void ChangeCharactor(int index)  
///       {
///          //キャラクター情報を変更します
/// 　       m_CurrentCharactor = (CharactorType)index;
///       }   
///  }
///                      ボタンのインスペクター上(On Click)    
///        // 通常 //　　                                //　EnumAction //                  
/// 　　[A.ChangeCharactor]                             [A.ChangeCharactor]
///                [     0]                                        [Player]  ←インスペクター上からは列挙子で表示される
/// 
/// </summary>
